...
async def render_edit_todo_page(request: Request,todo_id: int, db:db_dependency):
    try:
        ...
        return redirect_to_login()
@router.get("/")
async def read_all(user: user_dependency, db: db_dependency):#bunu çalıştırmak için bir database ve bir usera ihtiyacın var
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    return db.query(Todo).filter(Todo.owner_id == user.get("id")).all#veritabanında sadece o
                                                                # kullanıcının  tüm todolarını getirir
...
async def create_todo(user: user_dependency, db: db_dependency, todo_request: TodoRequest):
    ...
    db.add(todo)
    db.commit()#adden sonra commit yazmazsak işlem yapılmaz
...
def create_todo_with_gemini(todo_string:str):
    ...
    response = llm.invoke(
        [
            HumanMessage(content="I will provide you a todo item to add my to do list. What I want you to do is to create a longer and more comprehensive description of that to do item, my next message will be my todo: "),
